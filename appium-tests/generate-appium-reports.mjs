...
const REPORT_FILE = 'appium-test-report.xlsx';
...
const testCases = [
  ...
  { testId: 'TC-AUTH-023', module: 'Authentication', feature: 'Signup Screen', name: 'Verify confirm password mismatch check', desc: 'Ensure signup confirm password mismatch displays red error helper text below confirmation field.' },
  ...
  { testId: 'TC-AUTH-055', module: 'Authentication', feature: 'Reset Password Form', name: 'Verify new password mismatch check', desc: 'Ensure password reset modal displays mismatch alert toast when new password confirmation differs.' },
  ...
  { testId: 'TC-AUTH-069', module: 'Authentication', feature: 'Session Management', name: 'Verify session retention across tabs', desc: 'Ensure opening new browser tab retains logged-in user profile session via token.' },
  { testId: 'TC-AUTH-070', module: 'Authentication', feature: 'Session Management', name: 'Verify dual tabs logout synchronization', desc: 'Ensure triggering logout on session A dispatches broadcast channel event to immediately sign out session B.' },
  ...
  { testId: 'TC-DOCS-026', module: 'Documents', feature: 'Document List', name: 'Verify priority warning badge star render', desc: 'Ensure priority flag document cards in main grid view display prominent star icon overlay.' },
  ...
  { testId: 'TC-DOCS-080', module: 'Documents', feature: 'Edit Document', name: 'Verify close edits form actions', desc: 'Ensure clicking cancel on edit document form closes modal and retains original document attributes.' },
  ...
  { testId: 'TC-FAM-035', module: 'Family', feature: 'Member Actions', name: 'Verify cancel edits form close actions', desc: 'Ensure clicking cancel on edit family member form closes modal and retains original profile details.' },
...
async function generateReport() {
  console.log(`Generating Appium Test Report... Target cases count: ${testCases.length}`);
  ...
  headerRow.eachCell((cell) => {
    cell.fill = {
      type: 'pattern',
      pattern: 'solid',
      fgColor: { argb: 'FF8B5CF6' } // Purple accent for Appium
    };
  ...
  sortedCases.forEach((tc, idx) => {
    // Generate realistic randomized execution time and timestamp
